...
//
// **3つに分けてある**（2026-08-08。本人の指定）:
//
//   npm run bench          触ったときのもたつき（**約2.5分**。日常はこれ）
//   npm run bench:limits   ＋限界さがし（19軸）
//   npm run bench:export   ＋書き出し（60分ぶんを丸ごと焼く）
//
// **なぜ分けたか。** 全部入りの通しは、2026-08-08 までに**一度も最後まで行けなかった**。
// 60分の頭打ちを限界さがしで使い切り、書き出しに入る前にアプリごと落ちて
// 「✗ 書き出しが完走しなかった」——**アプリのせいに見える赤**が出ていた（測る側が
// 殺しただけ）。分ければ、それぞれ何分かかるかが実測できる（時間の内訳が出る）。
//
...
try {
  ...
  // **頭打ちは、走らせる中身に合わせる**（2026-08-08）。
  //
  // 60分固定にしていて、**通しが一度も最後まで行けなかった**。限界さがしで
  // 60分を使い切り、書き出しに入る前にアプリごと落ちて
  // **「✗ 60分ぶんの書き出しが完走する 完走しなかった」**——アプリのせいに見える
  // 赤が出ていた。**測る側が殺しただけ。**
  //
