...
/**
 * **その章だけを回す**（`--chapter=16-仕上げ`）。カンマで複数も可。
 * `--only` とは役目が違う:
 *
 *   --only=<語>       名前か章にその言葉を含む**確認**だけ（章はまたぐ）
 *   --chapter=<章,…>  その**章ファイルだけ**を、この順で読み込む
 *
 * ## 何のためか（2026-08-05）
 *
 * 通しは1つの Electron の中で18章を順に回すので、章が増えるほど伸びる。
 * 分散するために、まず**各章が前の章に寄りかかっていないか**を知る必要があった
 * （`e2e/solo-audit.mjs`。結果は18/18が独立）。
 *
 * ## カンマで複数を受けるのは「長く動かし続ける」面を残すため
 *
 * 1章＝1インスタンスにすると速いが、**1つのアプリを長く動かす面が消える**。
 * 直列の通しは13分ぶん同じインスタンスを使うので、状態の積み上がり
 * （片付け忘れ・溜まり続ける物）が出るならそこで出ていた。
 *
 * 担当ごとに数章をまとめて1インスタンスで回せば、両方取れる
 * ——1インスタンスあたり60項目前後を通しつつ、担当は並列。
 * 起動の回数も 18 → 担当の数まで減る。
 */
const CHAPTER = (process.argv.find((a) => a.startsWith('--chapter=')) ?? '')
  .slice(10)
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
