...
export function printSummary({ results, ONLY, CHANGED_INFO, viewWarnRef, ROOT }) {
  ...
  /**
   * **絞った語がどれにも当たらなかったら、緑で終わらせない**（2026-08-08）。
   *
   * 下ごしらえ（`opts.setup`）は絞り込みを素通りするので、語が1件も当たらなくても
   * **その数件だけが緑になり「全部通りました」と出る。** 実際にそうなった:
   *
   * ```
   * npm run e2e -- --only=見え方 --fast
   *   → 緑 9 / 赤 0 / 0.1分 / 「全部通りました」
   *   → 本題は **1件も走っていない**（`見え方` は節名でも確認名でもなく、
   *      章ファイルの名前とコメントにしか無い語だった）
   * ```
   *
   * 6秒で18章ぶん終わったのが手掛かりで、**数字を見なければ気づけなかった。**
   * 検査部の 4d「走らない見張りが、緑の顔をして残る」と同じ形なので、
   * **成立しなければ落とす**（CLAUDE.md 7番）。
   */
  // **「走った」かつ「下ごしらえではない」物だけを数える。**
  // 飛ばした物（skipped）を数に入れると、絞ったときは必ず大量にあるので
  // 判定が永久に成立しない＝**書いただけで効かない見張り**になる
  const 本題 = results.filter((r) => !r.setup && !r.skipped)
  if (ONLY.length && 本題.length === 0) {
    // **数は「走った物」で言う。** `results.length` には飛ばした物が入っているので、
    // それを「走った」と書くと**この見張り自身が嘘をつく**ことになる
    const 走った下ごしらえ = results.filter((r) => r.setup && !r.skipped).length
    console.log(
      `\n\x1b[31m「${ONLY.join(' / ')}」に当たる確認が1件もありませんでした。\x1b[0m` +
        `\n  走ったのは下ごしらえ ${走った下ごしらえ} 件だけ（${skipped} 件を飛ばしました）。` +
        `**何も確かめていません。**` +
        `\n  絞り込みは**確認名か節の名前**に当たる語で指定してください` +
        `（章ごと回すなら \`--chapter=<章>\`。章の一覧は e2e/lib/chapters.mjs）\n`
    )
    return -1
  }
