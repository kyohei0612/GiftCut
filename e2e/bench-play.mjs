 ...
 * ## 判定に使う数字
 *
 *   ずれ    再生ヘッドと時刻表示の差。**1.5コマ**を超えたら食い違い。
 *           1コマではない——**時刻表示はコマ単位に丸めてある**（`formatTimecode`）ので、
 *           連続値の再生ヘッドと比べれば最大1コマの差は必ず出る。実測でちょうど
 *           1.0コマ（33.5ms）が出て、線 33.3ms を 0.2ms 超えて赤になった。
 *           **丸めの分は「ずれ」ではない。** 2コマ離れたら人にも見える、が本当の線
...
async function 長く流してずれを見る({ page, shot, say, done, fmt, seekTo0, MINUTES }) {
  ...
  // ② 進み方（全部そろって遅れていないか）
  const 最後 = 有効[有効.length - 1]
  const 進んだ = 秒へ(最後.tc) - (秒へ(最初.tc) ?? 0)
  const 遅れ率 = 1 - 進んだ / 最後.経過
  ...
  const 行 =
    ...
    `${fmt(最後.経過)}秒で ${fmt(進んだ)}秒 進んだ（遅れ ${fmt(遅れ率 * 100)}%） / ` +
    `絵の固まり 最長 ${最長止まり}秒 / 時刻表示 ${最後.tc} / 標本 ${有効.length}` +
    (端A.最適化 != null || 端B.最適化 != null
      ? ` / **裏でプレビュー最適化中** ${端A.最適化 ?? '?'}% → ${端B.最適化 ?? '?'}%`
      : '')

  if (進んだ < 1) return done('動作', 名, `**再生が進んでいない**（${fmt(進んだ)}秒）。${行}`, 'ng')
  if (最長止まり >= 3)
