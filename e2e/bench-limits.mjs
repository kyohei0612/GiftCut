...
//
// ## ⚠ ここが測っているのは「平均」で、引っかかりは見えない（2026-08-04）
//
// この仕組みは長らく走っていなかった（本体の `video is not defined` で
// 手前で落ちていた）。動くようにしたら**全軸で傾きがほぼゼロ**と出たが、
// **同じ日に本体側は 95% が 4.4ms → 162.5ms（37倍）に跳ねていた。**
//
// 矛盾ではなく、測っている統計量が違う:
//
//   ここ    1操作あたりの**平均**（30回動かして割る＝約240ミリ秒ぶん）
//   本体    **95%・最悪・引っかかり回数**（26秒ぶん）
//
// **引っかかりは平均には出ない。** 中央値は重い条件でも 4.2ms（60fps）のままで、
// 跳ねるのは尾の方だけ。しかもここのドラッグは240ミリ秒しかないので、
// 26秒に165回起きる引っかかりを見る時間がそもそも無い。
//
// → **「この軸は平気」と読むときは「平均では」と補って読むこと。**
//   直すなら、ドラッグを長くして 95% を返す形にする（未着手）。
//
...
export async function findLimits({ ROOT, nowSec, say, done, app, fx, page, setZoom, heap, video, totalSec }) {
  ...
  // 何を1軸ずつ増やすかの表は ./bench-limits-axes（19軸）。
  // **ここは測る仕掛け**（増やして 95%・引っかかり・メモリを取る）で、話題が別。
  // 借りるのは作るのに時間がかかる素材だけ（画像・動画・別ファイル）。
  const sweeps = makeSweeps({ imgsByPx, imgs1920, vids3s, makeDistinctMedia })
