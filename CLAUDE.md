...
- **v0.1.11 を出した（2026-08-02）。** https://github.com/kyohei0612/GiftCut/releases
  自動更新が読む `latest.yml` が 0.1.11 を指していることまで確認済み
- `npm run verify` 緑（**698件**）。`npm run e2e` の通しは **08-02 昼が最後**（222 / 224）
  - その後、落ちていた2件の原因を直した（テロップの Ctrl+クリックが1個しか選べていなかった）。
    **絞った確認では約140件すべて緑だが、通しはまだ回していない**（本人の判断で見送り）
