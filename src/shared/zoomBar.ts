 ...
 *
 * ## 全体が収まった先も引ける（`floor` との小さい方）
 *
 * 短い素材では全体が収まる率の方が大きいので、下限は `floor`。
 * **全体が収まった後も、さらに引けて右に空白が出る。**
 *
 * ## 一度「全体が収まる所で止める」にして、戻した（2026-08-06）
 *
 * 止めた理由は「バーが表せないから」だった——つまみは見えている割合なので、
 * 全部見えた時点で満杯になり、その先は**倍率だけ下がってバーが動かない**。
 *
 * ところが本人が欲しかったのは逆で、**引き切った状態こそバーの満杯**だった:
 * 「バーのマックス状態を、かなり引いたタイムラインの状態にしてほしい」。
 *
 * → 限界は戻し、**バーの目盛りの方を伸ばした**（`barTotalSec`）。
 *   バーが「目一杯引いたときに見える範囲」を描くので、
 *   端＝限界の一致は保ったまま、引ける範囲を狭めずに済む。
 */
...
/**
 * **バーが描く範囲（秒）。目一杯引いたときに見える秒数。**
 *
 * ## なぜタイムラインの長さそのものではないか（2026-08-06・本人の指定）
 *
 * つまみは「バーの範囲のうち、いま見えている割合」。
 * 描く範囲をタイムラインの長さにすると、**全部見えた時点で満杯**になり、
 * そこからさらに引いても動かない（短い素材ではその区間が長い）。
 *
 * 描く範囲を「引き切ったときに見える秒数」にすると:
 *
 *   引き切った所      見えている＝バーの範囲そのもの → **満杯**
 *   全体が収まった所  見えている＝タイムラインの長さ → まだ余裕がある
 *
 * **バーの端と倍率の限界が一致し、その間も動く。**
 *
 * ※ 長い素材では下限が「全体が収まる率」なので、`viewW / lo` は
 *   タイムラインの長さとほぼ同じ＝**今までと変わらない**。伸びるのは短い素材だけ。
 */
export function barTotalSec(viewW: number, totalSec: number, floor: number): number {
  const lo = minZoom(viewW, totalSec, floor)
  const 引き切って見える秒 = lo > 0 ? viewW / lo : totalSec
  return Math.max(totalSec, 引き切って見える秒)
}
