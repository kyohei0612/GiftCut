...
/**
 * overlay の `enable=` に渡す式。**始まりを含み、終わりを含まない**。
 *
 * ## なぜ between ではないか
 *
 * ffmpeg の `between(t,a,b)` は両端を含む。隣り合う窓（前の end == 次の start）は
 * 境界のちょうど1フレームで両方とも真になり、二重に重なる。
 *
 * 以前はこれを「終わりを半フレーム詰める」で避けていた。しかしそれだと今度は
 * 窓と窓の間に**半フレームの隙間**ができる。動きの付いたテロップは出力fpsの刻みで
 * 短い窓を延々と並べるので、出力フレームがその隙間へ落ちるたびにテロップが消える。
 * 実際に「エフェクトを付けたテロップが書き出した動画でチカチカする」として出た。
 *
 * 半開区間なら、どの時刻もちょうど1つの窓にだけ入る。重ならず、隙間もできない。
 * 詰める必要そのものが無くなる。
 *
 * 返す文字列はフィルタへ直接埋める形（カンマは ffmpeg 向けにエスケープ済み）。
 */
export function overlayEnableExpr(start: number, end: number): string {
  return `gte(t\\,${start.toFixed(3)})*lt(t\\,${end.toFixed(3)})`
}
