 ...
 * **数えてから**割る（`引き継ぎ-App分割.md` の「境目をまたぐ名前 40個まで」）。
 ...
 * |---|---|---|---|
 * | `state/useAppWiring.tsx` | 1,250 | **境目をまたぐ名前 106〜413個**（2026-08-02 実測） | 目安の40個を大きく超える。割ると渡す物が爆発して、かえって読みにくくなる（`引き継ぎ-App分割.md` の段階4・5）。**先に受け取る側（context の束）を小さくする**のが順序 |
 * | `main/exportRun.ts` | 1,227 | 重ね合わせ 23個 / 切片の組み立て 21個 / ffmpeg 起動 24個 | どれも40個以内で**割れる**。ただし `filter` 文字列を書き換えて返す形なので、出すときは戻り値で組み直しが要る。次に着手するならここ |
 * | `e2e/run.mjs` | 1,114 | 道具の塊はまだ複数ある | 素材づくり・後片付け（`resetProject` まわり）が次の候補 |
 * | `e2e/checks/09-モーション.mjs` | 1,022 | 21項目→19項目 | 途中の塊を出すと**印の付いていない順番依存**がずれる（85 / 833 / 968行）。先にそこへ印を付けるのが順序 |
 */
