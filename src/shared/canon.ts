...
export const CANON: Canon[] = [
  ...
  },
  {
    ...
    // **返済済み**（2026-08-17。`useSegmentDrag` の写しを正典へ。
    // あれは端を掴んだあと後ろを詰める量で、下限が無いと逆へずれる形だった）
    debt: {}
  },
  {
    ...
    //
    // **残る2か所は 08-17 まで直っていなかった**（1年ではなく2週間、放置された）。
    // しかもその2つは**プレビューの本線**で、直っていた方（`clipXform`）が
    // 持っていたのは重ねた物（映像レイヤー・画像）だけ——
    // **使う人がいちばん触る所が、直っていない側に残っていた。**
    //
    //   `usePreviewFrame`  回転・反転を transform の**左**に置いていた
    //                      ＝反転した映像を右へ掴むと左へ動く
    //   `useSegClock`      式が写しなうえ**回転・反転が抜けていた**
    //                      ＝つなぎ目の間だけ反転が外れる
    //
    // 「下位の物が要る」と書いて見送っていたが、要ったのは
    // **繋ぎ方を持つ3行（`moveThen`）**だけだった。両方そこを通る。
  ...
  },
  {
    ...
    // 配線側で注釈を付け直さない）。経緯は `引き継ぎ-心臓の分け直し.md` の「6」。
    //
    // ※ 一段深い所（フックの deps）にあった同じ穴は **08-04 に返済済み**
    //   （221件 → 14件。残りは全部「正しい `any`」）。**数はここに書かない**
    //   ——`npm run measure` が数える。2026-08-17 まで「221件残っている」と
    //   書いたままで、**片付いた仕事を次の担当に読ませていた。**
  }
]
