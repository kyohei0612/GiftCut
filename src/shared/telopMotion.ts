...
export interface Motion {
  ...
  mbDir?: Keys
  /**
   * タービュレントディスプレイス（ぐにゃぐにゃ揺らす）の「適用量」（px・1080基準）。
   * 0 なら効果なし。**55.ホラーっぽい はこれが激しく上下するのが本体。**
   */
  tbAmt?: Keys
  /**
   * 同「サイズ」（px・1080基準）。揺れの粗さ。大きいほど大きくうねる。
   * **これは動かない**（実物でも固定値）ので、印ではなく数で持つ。
   */
  tbSize?: number
  /** 同「複雑度」（重ねる段数）。動かない設定値 */
  tbOct?: number
  /**
   * 同「ランダムシード」。**揺れの形そのもの**を選ぶ番号。
   * 14.揺れる動き_速 はここだけが動いて、形が次々に変わる＝揺れて見える。
   */
  tbSeed?: Keys
  /**
   * 同「オフセット」（px・1080基準）。揺れの模様そのものを平行移動する。
   * 24.タービュラント+光 はここが動いて、うねりが横へ流れていく。
   */
  tbOffX?: Keys
  tbOffY?: Keys
}
