...
// ※ 色とグラデ（hexToRgba / _lum / resolveGradMid / gradientStopStr /
//   isVerticalGrad / fillCss）は ./telopFill へ出した。**受け取る0・返す0**だった
...
// 色をどう混ぜて CSS にするかは ./telopFill に1つだけ（書き出しの SVG 側も同じ所を通る）
import { fillCss, hexToRgba, isVerticalGrad } from './telopFill'
...
export interface TelopStyle {
   ...
   * 縦書き。未指定＝横書き（いままでどおり）。
   *
   * **横書きの道すじには一切触らない。** 縦書きは `buildTelopSVG` の中で
   * 文字の置き方と枠の縦横だけを差し替える形にしてあり、縁・影・グラデ・
   * 部分装飾は同じ物がそのまま効く（＝画面と書き出しも同じ物を通る）。
   * 列は**右から左**へ並ぶ（日本語の縦組みの決まり）。
   */
  vertical?: boolean
  // アピアランス
