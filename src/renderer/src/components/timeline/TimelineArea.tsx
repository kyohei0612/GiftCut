...
// ## 中身
//
// - `TimelineArea` … これ1つだけ。**心臓を見に行く行が並び、そのあと絵**という形
//   （`use...Ctx()` が14本 → JSX を返す）。並ぶ順は
//   道具立て → 段の見出し → 本体（目盛り・再生ヘッド・帯・落とす影）→ 下の拡大バー
//
// **割っていないのは、絵が1枚だから。** 途中で切ると「どこに何を足すか」が
// かえって分からなくなる（切るなら帯の並びごと別部品へ出す）。
//
// ## 3層でできている
//
...
export function TimelineArea(): JSX.Element {
 ...
 segLayout, rulerTicks, padTop, padBottom, trackHOf, inView,
  ...
  // `currentTimeRef` は拡大バーへ（**値ではなく ref**。再生中に毎コマ描き直さない）
