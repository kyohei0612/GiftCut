...
export default function WaveformCanvas({
  ...
  useEffect(() => {
    ...
    // 組み直す（`Layerize`）のに時間を食う。組み直しは主スレッドの仕事の **85%** で、
    // **赤／緑がレイヤー枚数と完全に一致していた**（800枚→赤 / 160枚→緑）。
    // 塗る処理でも React（28秒中2.8秒）でもレイアウトでもない。
    //
    // 同じ物差しで両方測った（2026-08-04・`npm run bench --no-limits --no-export`）:
    //
    //                            GPU描画      CPU描画
    //   クリップを掴んで動かす    154.3ms  →   **70.7ms**
    ...
    //
    // **全項目で速い。** 拡大は「CPU で描き直すぶん遅くなるのでは」と疑ったが逆だった
