    ...
    <div className="panel-body" ref={bodyRef}>
      ...
      {library.length === 0 ? (
        ...
        <>
          ...
          {favList.length > 0 &&
            accSec('se', 'fav', '★ お気に入り', favList.length, (
              <div className="se-list">{favList.map(row)}</div>
            ))}
          {folders.map((f) => {
            const list = library.filter((s) => effective(s) === f.key)
            return accSec(
              'se',
              f.key,
              `📁 ${f.label}`,
              list.length,
              list.length ? (
                <div className="se-list">{list.map(row)}</div>
              ) : (
          ...
          {cats.map((cat) => {
            const list = library.filter((s) => effective(s) === cat)
            if (!list.length) return null
            return accSec('se', cat, `📁 ${cat}`, list.length, (
              <div className="se-list">{list.map(row)}</div>
            ))
          })}
        </>
