...
/**
 * 既定で高くする段。**本編（V1 / A1）だけ。**
 *
 * A1 は前から 44（波形の山が読める高さ。上のコメント参照）。
 * **V1 を足した（2026-08-05）。** 全部の段が同じ高さだと、どれが編の背骨なのかが
 * 絵から分からない——負荷テストの画面で、本編と重ねた素材が同じ重さで並んで
 * 「どこを見ればいいか」が読めなかった（UI-UX-見直し.md 2-8）。
 * プレミアも V1/A1 を既定で高くして、他を静かにしている。
 *
 * **2本だけにする。** 全部を高くすると合計が伸びて、タイムラインを縮めたときに
 * 縦へ溢れる（音声を全部高くしない理由と同じ）。ここが増えるほど効果は薄れる
 * ——「本編だけ高い」から背骨に見えるので、高い段が3本4本と増えたら意味が消える。
 */
export const DEFAULT_LANE_H: Record<string, number> = { V1: 44, A1: 44 }
