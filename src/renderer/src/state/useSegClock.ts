  ...
  function startVideoSegClock(): void {
    ...
    const tick = (): void => {
      ...
      if (src && pos < videoTLenRef.current - 1e-3) {
        ...
        //
        // **飛んでいる最中は数えない**（2026-08-16）。シーク中や、再生ヘッドを
        // 別の場所へ投げた直後は、`currentTime` がまだ前の場所を指しているので
        // 差が何十秒にもなる。**それは「遅れ」ではなく「移動中」。**
        // 数えると報告のいちばん上が「絵が遅れている（最大102秒）」になり、
        // **本当は15秒しか再生していない記録でも警告が出る**（実際に出た）。
        if (!vv.seeking && Math.abs(drift) < 5) perf.reportLag(drift)
