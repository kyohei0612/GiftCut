...
//
// ## 束を組み立てるのは、このファイルの `useTimelineOpsValue`
//
// **配線（useAppWiring）は呼ぶだけ**で、中身を1つも知らない。前は配線が81個の
// 名前を集めて手で組んでいたが、そうすると「この束に何を足すか」を決める場所と
// 中身が別のファイルに分かれて、両方を開かないと足せなかった（2026-08-04 に寄せた）。
//
// **`<TimelineOpsProvider>` の中で組んではいけない。** 囲いの中で作ると、
// 描き直しのたびに作り直されて、掴んでいる途中の状態が消える。
// 組むのは囲いの**外**（＝配線が `useTimelineOpsValue()` を呼んで value に渡す）。
