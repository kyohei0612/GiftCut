...
export function useTelopTab() {
  const {
    rightBodyRef, localTemplates, allCats, openTplSec, tplSecRefs, isFav, catOf,
    toggleTplSec, saveCurrentAsTemplate, addCustomCat, deleteCustomCat, refreshPresets,
    applyTemplate, deleteUserTemplate, toggleFav, setTplCat, setTplMenu, draggingTemplateRef
  } = useRightPanel()
