// 焼き直し（プロキシ）——プレビュー用に、全コマがキーフレームの映像を作る。
//
// ## なぜ要るか
//
...
// ## なぜ「素材の下ごしらえ」から出したか
//
// 元は `mediaProbe.ts` に、尺・コマ数を尋ねる所／サムネ／波形／無音探しと
// 同居していた。**あのファイルの頭のコメントが説明していたのは、ほぼ焼き直しだけ**で
// （x264 の事故の話）、他の話題は1行も書かれていなかった。
// 使う定数（proxyDir・上限・使用中の印・掃除）も全部この話題専用
...
/** 焼き直しの受け口。**app.whenReady() の中で1回だけ呼ぶ。** */
export function registerMediaProxyHandlers(): void {
