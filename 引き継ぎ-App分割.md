...
最終更新 2026-08-02。**11,404 → 380行**（配線 1,294行は state/useAppWiring.tsx へ）。

---

## いまの形（2026-08-02）

  App.tsx (380)              画面は何でできているか（心臓を積む・区画を並べる）
  state/useAppWiring (1,294) それらをどうつなぐか（どのフックに何を渡すか）
  state/use*.ts (約60個)     話題ごとの中身
  components/                区画・帯・ダイアログ

**配線の行数自体は減っていない。** 別ファイルへ移しただけ。
1ファイルのままなのは**フックどうしが互いを必要としているため**
（開く側と保存する側が互いの関数を要り、片方を「呼ぶときに見に行く」形で
輪を解いている）。ここをほどけば配線も話題ごとに割れる。
**ほどく前に無理やり割ると、渡す物が増えて読みにくくなるだけ。**
...
1. フックどうしの絡まりをほどく → 配線を話題ごとに割る
2. 段の太さの「前の既定からの引っ越し」（やること.md 参照。復活させず初期化ボタンにする案）
